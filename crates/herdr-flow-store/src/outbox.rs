...
pub fn reconcile_publication(
    ...
    let claimed = claim_next_intent(store, run_id, claim_owner)?;
    ...
    {
        release_if_known(store, &intent, claim_owner, effect_unknown)?;
    ...
    match intent.kind {
        PublicationIntentKind::PushRef => {
            mark_effect_unknown(store, &intent, claim_owner)?;
            let result = match provider.push_ref(&intent) {
                ...
                Err(error) => {
                    ...
                    {
                        release_fresh_uncertain(store, &intent, claim_owner)?;
                    }
                    return Err(StoreError::PublicationProvider(error));
                }
            };
            ...
            complete_intent(store, &intent, claim_owner, &result)?;
        ...
        PublicationIntentKind::CreateChangeRequest => {
            ...
            let result = match found {
                ...
                None => {
                    mark_effect_unknown(store, &intent, claim_owner)?;
                    match provider.create_change_request(&intent) {
                        Ok(result) => result,
                        Err(error) => {
                            if !effect_unknown
                                && !matches!(error, PublicationProviderError::EffectUnknown(_))
...
fn claim_next_intent(
    ...
    let row: Option<ClaimedIntentRow> = transaction
        ...
        .optional()
        .map_err(StoreError::Sqlite)?;
    let Some((operation_id, ordinal, digest, json, status, existing_owner)) = row else {
        transaction.commit().map_err(StoreError::Sqlite)?;
        return Ok(None);
    };
    if matches!(status.as_str(), "CLAIMED" | "EFFECT_UNKNOWN")
    ...
    if status == "PENDING" {
        let updated = transaction
            .execute(
                "UPDATE publication_outbox SET status = 'CLAIMED', claim_owner = ?1
...
fn release_if_known(
    ...
    if effect_unknown {
        ...
        release_claim(store, intent, claim_owner)
    }
}
...
mod tests {
    ...
    fn fake_provider_reconciles_uncertain_push_and_change_request_without_duplication() {
        ...
        provider.create_change_request(&change).unwrap();
        assert!(provider.find_change_request(&change).unwrap().is_some());
    ...
    fn intent_commitment_is_durable_idempotent_and_conflict_detecting() {
        ...
        let mut store = SqliteStore::open(directory.path().join("outbox.sqlite3")).unwrap();
        let push = intent(PublicationIntentKind::PushRef);
        store
        ...
        assert_eq!(
            claim_next_intent(&mut store, &push.run_id, &claim).unwrap(),
            Some((push.clone(), false))
        );
        let other_claim = operation("01ARZ3NDEKTSV4RRFFQ69G5FB1");
        assert!(matches!(
            claim_next_intent(&mut store, &push.run_id, &other_claim),
            Err(StoreError::PublicationOutboxNotReady)
        ));
        mark_effect_unknown(&mut store, &push, &claim).unwrap();
        assert_eq!(
            claim_next_intent(&mut store, &push.run_id, &claim).unwrap(),
            Some((push.clone(), true))
        );
        release_if_known(&mut store, &push, &claim, true).unwrap();
        ...
        assert_eq!(uncertain[0].claim_owner, Some(claim.clone()));
        complete_intent(
            &mut store,
            &push,
            &claim,
